...
func _process(delta):
	if cd>0:
		cd-=1
	elif (Input.is_action_pressed("Spara")):
...
func _physics_process(delta):
	var mov = Vector2(0, Input.get_axis("accelerazione", "freni"))
	velocity += mov.rotated(rotation)*accel*delta
	velocity=velocity.limit_length(maxvel)
	rotate(deg_to_rad(rotvel*delta*(int(Input.is_action_pressed("Vira a destra"))-int(Input.is_action_pressed("Vira a sinistra")))))
	move_and_slide()
